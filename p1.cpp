...
int generatePath() {
    ...
    map<tuple<pair<int, int>, int, int, int>, bool> visited;
    //for the initial node it is made true
    visited[make_tuple(start_pos, 0, 0, 0)] = true;
    visited[make_tuple(start_pos, 0, 1, 0)] = true;
    visited[make_tuple(start_pos, 0, 2, 0)] = true;
    visited[make_tuple(start_pos, 0, 3, 0)] = true;
    ...
    while (!q.empty()) {
        ...
        if (moves<2){
            cout << "testing " << moves << " " << pos.first << "," << pos.second << " ";
            cout << speed << " " << dir << "\n";
        }
                    ...
                    if (!visited[make_tuple(new_pos, s, dir, new_c_score)]) {
                        ...
                        if (s == 0) {
                            for (int new_dir : {0, 1, 2, 3}) {
                                ...
                                if (!visited[new_pos_dir]) {
                                    visited[new_pos_dir] = true;
